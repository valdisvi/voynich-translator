...
public class SwingController {
		...
		try {
			...
			if (name.equals("Currier.properties") || name.equals("BasicEVA_to_ASCIIsounds.properties")
				...
				if (check == JOptionPane.YES_OPTION) {
					try {
						//this is a java bug.
						//manual call to a garbage collector necessary to delete file
				        System.gc();
						Files.delete(p);
	...
	protected void getResult(JTextArea t, JTextPane text, String author, ButtonGroup group) {
		...
		String f = "f0";
		String webpage = "";
		//String webpage = "http://www.voynich.nu/q01/f00" + t.getText() + version + "_tr.txt"; // source
		//authorURL = webpage;
		try {
		...
		//all of the other quires have varying amount of pages, hence the coding
		//so blame the source website, not the poor coder that had to write this
		if (page >= 57 && page <= 66 ) {
		   quire = "q08";
		  // f = "f00";
		}
		...
		try {
			...
			if (author == null) {
				JOptionPane.showMessageDialog(null, "No author selected.\nPlease choose an author from a list",
						"Error", JOptionPane.ERROR_MESSAGE);
			} else if (text.getText().equals("")) {
				JOptionPane.showMessageDialog(null, "Could not transliterate the page. Check if this author \n has transcribed this page.", "Error",
						JOptionPane.ERROR_MESSAGE);
			}
			...
			JOptionPane.showMessageDialog(null,
					"Page "+ t.getText() + version + " does not exist (is missing) \n or there is a problem with the connection.", "Error",
					JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	public void setFont(JButton b, JTextPane text, JTable table) throws FontFormatException, IOException {
		Font v = Font.createFont(Font.TRUETYPE_FONT, getClass().getClassLoader().getResourceAsStream("application/voynich.ttf")).deriveFont(12f);
		GraphicsEnvironment ge = GraphicsEnvironment.getLocalGraphicsEnvironment();
		ge.registerFont(Font.createFont(Font.TRUETYPE_FONT, getClass().getClassLoader().getResourceAsStream("application/voynich.ttf")));
